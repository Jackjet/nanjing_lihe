  ...
  <el-main class="main">
    <el-col :span="24" class="center">
      ...
      <el-table
        ...
        class="table-list"
        :cell-style="{'font-size': '14px','height': '40px', 'padding': '0'}"
        :header-cell-style="{'font-size': '14px', 'height': '40px', 'padding': '0', 'background-color': '#F5F7FA'}"
        row-key="id"
        ...
        <el-table-column align="center" label="操作">
          <template slot-scope="scope">
            <el-button  v-if="scope.row.type !== '按钮' && permissions.add && scope.row.hasSystem === '0'"
                        type="text"
                        size="medium"
                        icon="el-icon-plus"
                        @click="handleAdd(scope.row)">新增
            </el-button>
            <el-button  v-if="permissions.edit && scope.row.hasSystem === '0'"
                        type="text"
                        size="medium"
                        icon="el-icon-edit-outline"
                        @click="handleEdit(scope.row)">编辑
            </el-button>
            <el-button  v-if="permissions.delete && scope.row.hasSystem === '0'"
                        type="text"
                        size="medium"
                        icon="el-icon-delete"
                        @click="handleSigleDelete(scope.row)">删除
            </el-button>
          </template>
        </el-table-column>
      </el-table>
