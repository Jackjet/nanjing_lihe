<template>
  <el-main>
    ...
    <el-col class="main-center">
      <el-form :model="ruleForm" v-loading="isSubmitLoading" :rules="rules" ref="ruleForm" label-width="150px" class="demo-ruleForm">
        <el-form-item label="名称:" :label-width="this.formLabelWidth" prop='name'>
          <el-input clearable maxlength='100' size="small" :disabled="disabled" placeholder="请输入名称" v-model="ruleForm.name"></el-input>
        </el-form-item>
        ...
        <el-form-item prop="icon" label="菜单图标:" :label-width="this.formLabelWidth" class='InpitWidth'>
          <el-select v-model="ruleForm.icon" style="width: 100%;" clearable :disabled='disabled' placeholder="请选择菜单图标">
            <el-option v-for="item in theIcon" :key="item.id" :label="item.name" :value="item.id">
              <div class="icon-box">
        ...
        <el-form-item v-show="ruleForm.type === '1'" label="页面地址:" :label-width="this.formLabelWidth">
          <el-input clearable maxlength='100' size="small" placeholder="请输入页面地址" v-model="ruleForm.path"></el-input>
        </el-form-item>
        <el-form-item v-show="ruleForm.type === '2'" label="绑定字段:" :label-width="this.formLabelWidth">
          <el-input clearable maxlength='100' size="small" placeholder="请输入绑定字段" v-model="ruleForm.path"></el-input>
        </el-form-item>
...
export default {
  ...
  created() {
    ...
    this.$router.options.routes.forEach(item => {
      if (item.path === '/quality' || item.path === '/government') {
