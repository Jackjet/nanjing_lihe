    ...
    <el-col :span="24" class="center">
      ...
      <el-table v-loading="isSubmitLoading" class="table-list" :cell-style="{'font-size': '14px','height': '40px', 'padding': '0'}" :header-cell-style="{'font-size': '14px', 'height': '40px', 'padding': '0', 'background-color': '#F5F7FA'}" row-key="id" @expand-change='expandChange' :expand-row-keys="expand" :data="tableData" style="width: 100%; margin-top: 10px;" stripe="true" indent="16" @selection-change="handleSelectionChange" :tree-props="{children: 'children', hasChildren: 'hasChildren'}">
        ...
        <el-table-column prop="seq" align="center" label="排序">
          <template slot-scope="scope">
            <template v-if="scope.row.seq === -1">
              <el-button type="text" size="small" icon="el-icon-top" @click="menuSeq(0,scope.row)"></el-button>
            </template>
            <template v-else-if="scope.row.seq === 0">
              <el-button type="text" size="small" icon="el-icon-bottom" @click="menuSeq(1,scope.row)"></el-button>
            </template>
        ...
        <el-table-column align="center" label="操作">
          <template slot-scope="scope">
            <el-button v-if="scope.row.type !== '按钮' && permissions.add && scope.row.hasSystem === '0'" type="text" size="small" icon="el-icon-plus" @click="handleAdd(scope.row)">新增
            </el-button>
            <el-button v-if="permissions.edit && scope.row.hasSystem === '0'" type="text" size="small" icon="el-icon-edit-outline" @click="handleEdit(scope.row)">编辑
            </el-button>
            <el-button v-if="permissions.delete && scope.row.hasSystem === '0'" type="text" size="small" icon="el-icon-delete" @click="handleSigleDelete(scope.row)">删除
            </el-button>
          </template>
        </el-table-column>
      </el-table>
...
export default {
  ...
  created() {
    ...
    if (sessionStorage.getItem("expand")) {
      this.expand = JSON.parse(sessionStorage.getItem("expand"))
    }
    this.fetchData()
  },
  methods: {
    expandChange(row) {
      ...
      sessionStorage.setItem("expand", JSON.stringify(this.expand))
    },
    async fetchData() {
      const data = {}
      const that = this
